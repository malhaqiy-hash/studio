...
export default function ScoutPage() {
  ...
  const runScout = async () => {
    setLoading(true);
    try {
      // In a real app, context would be fetched from Firestore analytics
      ...
      <div className="space-y-8 max-w-5xl mx-auto py-6">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl overflow-hidden relative">
          <div className="relative z-10 space-y-2">
            <div className="flex items-center gap-2 mb-2">
              <Badge className="bg-emerald-500 text-white border-none px-3 py-1 font-black text-[10px] uppercase tracking-widest">Market Intelligence</Badge>
              <Badge variant="outline" className="text-white border-white/20 text-[10px] uppercase font-bold">Scout V 1.0</Badge>
            </div>
            <h1 className="text-4xl font-black tracking-tight leading-none flex items-center gap-3">
              AI Business Scout
            </h1>
            <p className="text-slate-400 max-w-lg font-medium">Detecting unmet demand and hidden market gaps across the global network signals.</p>
          </div>
          <div className="relative z-10">
            ...
            >
              ...
              {loading ? "Scanning Network..." : "Start Scouting"}
            </Button>
          </div>
          ...
          <Card className="border-none shadow-sm bg-indigo-50/50 rounded-[2.5rem]">
            <CardContent className="p-16 text-center space-y-8">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-slate-900">Analyzing Unmet Demand</h3>
                <p className="text-slate-500 font-medium max-w-sm mx-auto">
                  Processing search logs, failed deals, and industry trends to find your next pivot...
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            {reports.reports.map((report, idx) => (
              <Card key={idx} className="overflow-hidden border-slate-100 shadow-xl rounded-[2.5rem] bg-white group hover:border-emerald-200 transition-all flex flex-col">
                ...
                <CardContent className="p-8 pt-2 flex-1 space-y-6">
                   <div className="space-y-2">
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Analysis Signal</span>
                   ...
                   <div className="p-4 bg-slate-50 rounded-2xl space-y-3">
                      <div className="flex items-center justify-between">
                         <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Potential Value</span>
        ...
        {!reports && !loading && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6">
            {[
              { icon: TrendingUp, title: "Detect Gaps", desc: "Find what users are searching for that doesn't exist yet." },
              { icon: Target, title: "Lead Generation", desc: "AI creates custom opportunities based on market shortages." },
              { icon: BarChart3, title: "Trend Analysis", desc: "Visualizing the next big B2B trade movements." }
            ].map((f, i) => (
